...
void CMesh::render()
{
	...
	for (unsigned int i = 0 ; i < m_Entries.size() ; i++) {
		...
		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(SVertex), 0);				//position	
		glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(SVertex), (const GLvoid*)12);//texture
		glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, sizeof(SVertex), (const GLvoid*)20);//normal
		glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, sizeof(SVertex), (const GLvoid*)32);//tangent
